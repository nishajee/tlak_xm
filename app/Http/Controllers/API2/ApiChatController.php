...
{   
    public function chatPeoplegroupList(Request $request){
        ...
        $tourPackage = TourPckage::where('id',$tour_package_id)
          ...
          if($tourPackage){
            $groupT = GroupTraveler::where('tour_package_id', $tourPackage->id)->select('type','tour_package_id as PackageID','group_unique_id as UniqueID','id as groupId','group_name as groupName')->first();

            $tra = Traveler::where('tour_package_id', $tour_package_id)
                ->select('id as travelerId','name as travelerName','profile_picture as profileImage','gender','tpassword as password','userid as quickUserID','occupant_id as occupantId','type')
                ->get(); 
                $groupst = array();
                foreach($tra as $key => $grp){
                    if($grp->profileImage == '' || $grp->profileImage == null) {
                        if($grp->gender == 'male'){
                          $gender = 'male';
                        }
                        elseif($grp->gender == 'female') {
                            $gender = 'female';
                        }
                        else{
                            $gender = 'undefined';
                        }
                        $rand_image = $this->getRandomImage($gender);
                        $profilePicture = "https://account.tlakapp.com/media/avatars/".$gender."/".$rand_image;
                    }
                    
                    else{
                        $profilePicture = 'https://s3.us-west-2.amazonaws.com/s3-tlak-bucket/traveller/profile/'.$grp->profileImage;
                    }

                    $occ_id = ($grp->occupantId != '' || $grp->occupantId != null) ? $grp->occupantId : '';

                    $password = ($grp->password != '' || $grp->password != null) ? $grp->password : '';

                    $quickId = ($grp->quickUserID != '' || $grp->quickUserID != null) ? $grp->quickUserID : '';

                    $aaa = ['travelerId'=>$grp->travelerId,'travelerName'=>$grp->travelerName,'password'=>$password,'quickUserID'=>$quickId,'occupantId'=>$occ_id,'type'=>$grp->type,'profilePicture'=>$profilePicture];
               ...
               $groupT['travellers'] = $groupst;
                ...
                ->where('type', "Manager")
                ->select('id as travelerId','name as travelerName','profile_picture as profileImage','gender as gender','tpassword as password','userid as quickUserID','occupant_id as occupantId','type')
                ->get();
                $tm = count($tourManagers);
             if($tm >= 1){
                foreach($tourManagers as $Tmgr){
                    if($Tmgr->profileImage == '' || $Tmgr->profileImage == null) {
                        if($Tmgr->gender == 'male'){
                          $gender = 'male';
                        }
                        elseif($Tmgr->gender == 'female') {
                            $gender = 'female';
                        }
                        else{
                            $gender = 'undefined';
                        ...
                        $profilePicture = 'https://s3.us-west-2.amazonaws.com/s3-tlak-bucket/traveller/profile/'.$Tmgr->profileImage;
                    }

                    $m_ooc_id = ($Tmgr->occupantId != '' || $Tmgr->occupantId != null) ? $Tmgr->occupantId : '';

                    $m_password = ($Tmgr->password != '' || $Tmgr->password != null) ? $Tmgr->password : '';

                    $m_quick_id = ($Tmgr->quickUserID != '' || $Tmgr->quickUserID != null) ? $Tmgr->quickUserID : '';

                    $tourManager[] = ['ChatType'=>"Individual",'travelerId'=>$Tmgr->travelerId,'travelerName'=>$Tmgr->travelerName,'password'=>$m_password,'quickUserID'=>$m_quick_id,'occupantId'=>$m_ooc_id,'type'=>$Tmgr->type,'profilePicture'=>$profilePicture];
                  }
               }
                else{
                  $tourManager = array();
            ...
            $inditravellers = Traveler::where('tour_package_id', $tour_package_id)
              ...
              if($it >= 1){
                foreach($inditravellers as $traveler){
                    if($traveler->profileImage == '' || $traveler->profileImage == null) {
                        if($traveler->gender == 'male'){
                          $gender = 'male';
                        }
                        elseif($traveler->gender == 'female') {
                            $gender = 'female';
                        }
                        else{
                            $gender = 'undefined';
                        ...
                        $profilePicture = 'https://s3.us-west-2.amazonaws.com/s3-tlak-bucket/traveller/profile/'.$traveler->profileImage;
                    }

                    $i_occ_id = ($traveler->occupantId != '' || $traveler->occupantId != null) ? $traveler->occupantId : '';

                    $i_password = ($traveler->password != '' || $traveler->password != null) ? $traveler->password : '';

                    $i_quick_id = ($grp->quickUserID != '' || $grp->quickUserID != null) ? $grp->quickUserID : '';

                    $individualTravellers[] = ['ChatType'=>"Individual",'travelerId'=>$traveler->travelerId,'travelerName'=>$traveler->travelerName,'password'=>$i_password,'quickUserID'=>$i_quick_id,'occupantId'=>$i_occ_id,'type'=>$traveler->type,'profilePicture'=>$profilePicture];
                }
              }
              else{
                $individualTravellers = array();
              }
